...
let () =
  for i = 0 to len0 - 1 do let yi,xi = a.(i) in
    for j = 0 to len0 - 1 do if i <> j then let yj,xj = a.(j) in
      if yi = yj && (xj = xi+2 && get g (yi, xi+1) = '#' ||
                     xj = xi-2 && get g (yi, xi-1) = '#')
      || xi = xj && (yj = yi+2 && get g (yi+1, xi) = '#' ||
                     yj = yi-2 && get g (yi-1, xi) = '#')
      then
        let save = j - i - 2 in
        if save >= 0 then (
